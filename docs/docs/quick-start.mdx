...
Use the `npx floww init` command to create a new project:
...
Edit the `main.ts` file created by `npx floww init`:
...
When you run `npx floww dev`, you'll get a webhook URL like:
```
